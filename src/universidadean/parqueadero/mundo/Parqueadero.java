...
public class Parqueadero {
     ...
     * @return Respuesta 1.
     *
     * debera retornar la placa del carro que lleva mas tiempo en el
     * parqueadero
     *
     */
    public String metodo1() {
        ...
        int tiempo = 0;
        Boolean sw = false;
        for (Puesto posicion : puestos) {
            if (posicion.estaOcupado()) {
                sw = true;
                Carro vehiculo = posicion.darCarro();
                if (vehiculo.darTiempoEnParqueadero(vehiculo.darHoraLlegada()) > tiempo) {
                    placaCarroConMasTiempo = vehiculo.darPlaca();
                    return "la placa del carro que lleva mas tiempo en el parqueadero es "+placaCarroConMasTiempo;
                }
            }
    ...
    /**
     * Metodo de extension 2.
     *
     * @return Respuesta 2.
     */
    public String metodo2() {
        int nPuestosTotales = TAMANO;
        int nPuestosOcupados = 0;
        for (Puesto puesto:puestos){
            if (puesto.estaOcupado()){
                nPuestosOcupados += 1;
            }
        }
        if (nPuestosOcupados >= nPuestosTotales){
            return "El parqueadero esta lleno";
        }else{
            return String.valueOf("Puestos ocupados : "+nPuestosOcupados+"\nHay puestos disponibles");
        }
    }
